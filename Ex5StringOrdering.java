...
public class Ex5StringOrdering {
    ...
    boolean isOrdered(String order, String tokens) {
        char [] orderofLetters = order.toCharArray();
        for (int i = 1; i < order.length(); i ++){
            if (tokens.isEmpty()){
                return true;
            }
            if (!(tokens.lastIndexOf(orderofLetters[i-1]) < tokens.indexOf(orderofLetters[i]))){
                return false;
            }
        }return true;
    }
